...
public class MapPropertyDescriptor extends GetterSetterPropertyDescriptor {
    ...
    public Method getWriteMethod() throws NoSuchMethodException {
        if (MappingUtils.isBlankOrNull(setMethodName)) {
            throw new MappingException(
                "Custom Map set method not specified for field mapping to class: " + clazz + ".  Perhaps the map-set-method wasn't specified in the dozer mapping file?");
        }
    ...
    public void setPropertyValue(Object bean, Object value, FieldMap fieldMap) {
        if (MappingUtils.isDeepMapping(fieldName)) {
            ...
            if (!getPropertyType().isPrimitive() || value != null) {
                // Check if dest value is already set and is equal to src value.
                // If true, no need to rewrite the dest value
                try {
                    ...
                    // if we failed to read the value, assume we must write, and
                    // continue...
                }
    ...
    protected Method getReadMethod() throws NoSuchMethodException {
        if (MappingUtils.isBlankOrNull(getMethodName)) {
            throw new MappingException(
                "Custom Map get method not specified for field mapping to class: " + clazz + ".  Perhaps the map-get-method wasn't specified in the dozer mapping file?");
        }
