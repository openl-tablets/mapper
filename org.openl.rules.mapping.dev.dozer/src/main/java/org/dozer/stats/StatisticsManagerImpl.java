...
/**
 * Internal class that manages the Dozer runtime statistics. Only intended for
 * internal use.
 *
 * @author tierney.matt
 */
public final class StatisticsManagerImpl implements StatisticsManager {
    ...
    /*
     * Convenience method that should only be used for statistic types that will
     * only ever have 1 statistic entry(value). For stats that only have one
     * entry, it is assumed that the single entry's key is the same as the stat
     * type name
     */
    ...
    protected Statistic increment(StatisticType statisticType, Object statisticEntryKey, long value) {
        ...
        // Get Statistic object for the specified type. If it doesn't already
        // exist, create it
        Statistic statistic = statisticsMap.get(statisticType);
        if (statistic == null) {
    ...
    /*
     * Convenience method that should only be used for statistic types that will
     * only ever have 1 statistic entry(value). getStatisticEntries() should be
     * used for statistic types that have more than 1 statistic entry(value)
     */
