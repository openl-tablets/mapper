...
/**
 * Internal class that performs one time Dozer initializations. Only intended
 * for internal use. Registers internal JMX MBeans if those are enabled in the
 * configuration.
 *
 * @author tierney.matt
...
public final class DozerInitializer {
    ...
    public void init() {
        ...
        synchronized (this) {
            ...
            log.info("Initializing Dozer. Version: {}, Thread Name: {}",
                DozerConstants.CURRENT_VERSION,
    ...
    void initialize(GlobalSettings globalSettings) {
        if (globalSettings.isAutoregisterJMXBeans()) {
            // Register JMX MBeans. If an error occurs, don't propagate
            // exception
            try {
                registerJMXBeans(new JMXPlatformImpl());
            } catch (Throwable t) {
                log.warn(
                    "Unable to register Dozer JMX MBeans with the PlatformMBeanServer.  Dozer will still function " + "normally, but management via JMX may not be available",
                    t);
            }
        }
        ...
        Class<? extends DozerClassLoader> classLoaderType = loadBeanType(classLoaderName,
            defaultClassLoader,
            DozerClassLoader.class);
        Class<? extends DozerProxyResolver> proxyResolverType = loadBeanType(proxyResolverName,
            defaultClassLoader,
    ...
    private <T> Class<? extends T> loadBeanType(String classLoaderName,
    ...
    /**
     * Performs framework shutdown sequence. Includes de-registering existing
     * Dozer JMX MBeans.
     */
    public void destroy() {
        synchronized (this) {
    ...
    private void registerJMXBeans(JMXPlatform platform) throws MalformedObjectNameException,
    ...
    private void unregisterJMXBeans(JMXPlatform platform) throws MBeanRegistrationException,
