 ...
 * validations on the explicit field mappings. It applies global configuration
 * and class level attributes to raw class mappings. It also creates the
 * ClassMap "prime" instance for bi-directional mappings. The ClassMap prime is
 * created by copying the original ClassMap and reversing the attributes. Only
...
public final class MappingsParser {
    ...
    public ClassMappings processMappings(List<ClassMap> classMaps, Configuration globalConfiguration) {
        ...
        for (ClassMap classMap : classMaps) {
            ...
            // add our first class map to the result map and initialize
            ...
            // Check to see if this is a duplicate map id, irregardless of src
            // and dest class names.
            ...
            if (classMap.getFieldMaps() != null) {
                ...
                for (Object fm1 : fms) {
                    ...
                    // field map into a MapFieldMap type
                    ...
                    // iterate through the fields and see whether or not they
                    // should
                    // be mapped
                    ...
                    if (!MappingDirection.ONE_WAY.equals(classMap.getType())) {
                        ...
                        if (!(MappingDirection.ONE_WAY
                            ...
                            // iterate through copyByReferences and set
                            // accordingly
                            if (!(fieldMap instanceof ExcludeFieldMap)) {
                            ...
                            // if it is a one-way field map make the other
                            // field map excluded
                            // make a prime field map
                            if (!(fieldMap instanceof MultiSourceFieldMap)) {
                        ...
                        // since it is one-way...we still need to validate if it
                        // has
                        // some type of method mapping and validate the
                        // field maps
                        FieldMap oneWayFieldMap = fieldMap;

                        if (oneWayFieldMap instanceof MultiSourceFieldMap && StringUtils.isEmpty(oneWayFieldMap
                            .getCustomConverter()) && StringUtils.isEmpty(oneWayFieldMap.getCustomConverterId())) {

                            MappingUtils
                                .throwMappingException("custom converter should be provided for n to 1 field mapping");
                        }
            ...
            // if it is a one way mapping or a method/iterate method mapping we
