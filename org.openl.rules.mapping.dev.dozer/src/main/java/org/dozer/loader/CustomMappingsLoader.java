...
/**
 * Internal class that loads and parses custom xml mapping files into ClassMap
 * objects. The ClassMap objects returned from the load method are fully
 * decorated and ready to be used by the internal mapping engine. Only intended
 * for internal use.
 * 
 * @author tierney.matt
...
public class CustomMappingsLoader {
    ...
    public LoadMappingsResult load(List<String> mappingFiles, List<MappingFileData> builderMappings) {
        ...
        // Decorate the raw ClassMap objects and create ClassMap "prime"
        // instances
        ...
        // is true. The addDefaultFieldMappings will check the wildcard policy
        // of each classmap
        ...
        if (globalConfiguration.getCustomConverters() != null && globalConfiguration.getCustomConverters()
            .getConverters() != null) {
        ...
        // iterate through the classmaps and set all of the customconverters on
        // them
            ...
            if (mappingFileData.getConfiguration() != null) {
                ...
                if (globalConfiguration != null) {
                    MappingUtils.throwMappingException(
                        "More than one global configuration found.  " + "Only one global configuration block (<configuration></configuration>) can be specified across all mapping files.  " + "You need to consolidate all global configuration blocks into a single one.");
                }
