...
/**
 * Internal singleton class that holds the global settings used by Dozer. Most
 * of these settings are configurable via an optional Dozer properties file. By
 * default, Dozer will look for a file named dozer.properties to load these
 * configuration properties. If a properties file is not found or specified,
...
public class GlobalSettings {
    ...
    private synchronized void loadGlobalSettings() {
        ...
        // Load prop file. Prop file is optional, so if it's not found just use
        // defaults
        DozerClassLoader classLoader = BeanContainer.getInstance().getClassLoader();
        URL url = classLoader.loadResource(propFileName);
        if (url == null) {
            log.warn("Dozer configuration file not found: {}.  Using defaults for all Dozer global properties.",
                propFileName);
            return;
        } else {
    ...
    private void populateSettings(Properties props) {
        ...
        if (propValue != null) {
            statisticsEnabled = Boolean.valueOf(propValue); // TODO Parsing
                                                            // errors?
        }
