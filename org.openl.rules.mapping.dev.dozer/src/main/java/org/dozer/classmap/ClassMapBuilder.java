...
/**
 * Internal class for adding implicit field mappings to a ClassMap. Also, builds
 * implicit ClassMap for class mappings that don't have an explicit custom xml
 * mapping. Only intended for internal use.
 *
 * @author tierney.matt
...
public final class ClassMapBuilder {
    ...
    /**
     * Builds new default mapping on-the-fly for previously unknown mapped class
     * pairs.
     *
    ...
    public static ClassMap createDefaultClassMap(Configuration globalConfiguration,
        ...
        classMap.setSrcClass(new DozerClass(srcClass.getName(),
            srcClass,
            ...
            null,
            null,
            DozerConstants.DEFAULT_MAP_NULL_POLICY,
            DozerConstants.DEFAULT_MAP_EMPTY_STRING_POLICY,
            false));
        classMap.setDestClass(new DozerClass(destClass.getName(),
            destClass,
            globalConfiguration.getBeanFactory(),
            null,
            null,
    ...
    static boolean shouldIgnoreField(String fieldName, Class<?> srcType, Class<?> destType) {
        ...
        if ((CALLBACK.equals(fieldName) || CALLBACKS.equals(fieldName)) && (MappingUtils
            .isProxy(srcType) || MappingUtils.isProxy(destType))) {
            return true;
        }
        return false;
    }

    private static void addFieldMapping(ClassMap classMap,
            Configuration configuration,
            String srcName,
            String destName) {
    ...
    private static void addGenericMapping(ClassMap classMap,
            Configuration configuration,
            String srcName,
            String destName) {
        FieldMap fieldMap = new GenericFieldMap(classMap);
    ...
    private static void validate(Mapping annotation, Member member) {
        if (annotation.value().trim().equals("")) {
            throw new MappingException(
                "Mapping annotation value missing at " + member.getDeclaringClass().getName() + "." + member.getName());
        }
    }
    ...
    public static class MapMappingGenerator implements ClassMappingGenerator {
        ...
        public boolean accepts(ClassMap classMap) {
            ...
            return MappingUtils.isSupportedMap(srcClass) || classMap.getSrcClassMapGetMethod() != null || MappingUtils
                .isSupportedMap(destClass) || classMap.getDestClassMapGetMethod() != null;
        }
        ...
        public boolean apply(ClassMap classMap, Configuration configuration) {
            ...
            for (PropertyDescriptor property : properties) {
                ...
                if (StringUtils.isNotEmpty(classMap.getSrcClassMapGetMethod()) || StringUtils
                    .isNotEmpty(classMap.getSrcClassMapSetMethod())) {
                ...
                if (StringUtils.isNotEmpty(classMap.getDestClassMapGetMethod()) || StringUtils
                    .isNotEmpty(classMap.getDestClassMapSetMethod())) {
    ...
    public static class BeanMappingGenerator implements ClassMappingGenerator {
        ...
        public boolean apply(ClassMap classMap, Configuration configuration) {
            ...
            for (PropertyDescriptor destPropertyDescriptor : destProperties) {
                ...
                if (classMap.getFieldMapUsingDest(fieldName) != null || classMap
                    .getFieldMapUsingSrc(fieldName) != null) {
                    continue;
                }
